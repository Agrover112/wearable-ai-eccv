...
DEV_RUN="$PROJECT_ROOT/runs/egolongqa/qwen3_5_0_8b_hf_images64_temporal_pivot_c128_dev140_local_sdpa_2026-07-30"
RUN_DIR="$PROJECT_ROOT/runs/egolongqa/qwen3_5_0_8b_hf_images64_temporal_pivot_c128_full700_local_sdpa_2026-07-31"
...
export QWEN35_ATTN_IMPLEMENTATION="${QWEN35_ATTN_IMPLEMENTATION:-sdpa}"
...
# Generate only the held-out rows while reusing the populated dev140 feature cache
python baselines/longqa/run_generate_longqa_proofpack.py \
  ...
  --grounding-output "$RUN_DIR/val560_proofpack.jsonl" \
  --no-eval
...
python baselines/longqa/merge_longqa_shards.py \
  --reference "$ANNOTATIONS" \
  --shard "$DEV_RUN/proofpack.jsonl" \
  --shard "$RUN_DIR/val560_proofpack.jsonl" \
  --output "$RUN_DIR/proofpack.jsonl"
