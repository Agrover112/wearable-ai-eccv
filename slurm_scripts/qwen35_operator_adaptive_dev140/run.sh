#!/usr/bin/env bash
#SBATCH --job-name=q35-operator-adaptive-dev140
#SBATCH --partition=gpu24
#SBATCH --gres=gpu:h100:1
#SBATCH --time=04:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --output=slurm_outputs/qwen35_operator_adaptive_dev140/%x_%j.out
#SBATCH --error=slurm_outputs/qwen35_operator_adaptive_dev140/%x_%j.err

set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:-${SLURM_SUBMIT_DIR:-$(pwd)}}"
RUN_NAME="${RUN_NAME:-qwen35_operator_adaptive_dev140}"
RUN_DIR="$PROJECT_ROOT/runs/egolongqa/$RUN_NAME"

PARENT_PROOFPACK="${PARENT_PROOFPACK:-$PROJECT_ROOT/runs/egolongqa/qwen35_parent_temporal_pivot_dev140/proofpack.jsonl}"
if [[ ! -f "$PARENT_PROOFPACK" ]]; then
    echo "PARENT_PROOFPACK does not exist: $PARENT_PROOFPACK" >&2
    exit 2
fi
...
mkdir -p "$RUN_DIR" "$VLLM_LOG_DIR" \
    "$PROJECT_ROOT/slurm_outputs/qwen35_operator_adaptive_dev140"

echo "Job ID: ${SLURM_JOB_ID:-local}"
echo "Parent proof pack: $PARENT_PROOFPACK"
echo "Run directory: $RUN_DIR"
echo "Started at: $(date)"

python baselines/longqa/run_generate_longqa_operator_adaptive.py \
    ...
    --parent-proofpack "$PARENT_PROOFPACK" \
